...
export function OrdenPagosSection({ ordenId, ordenTotal, status }: Props) {
  ...
  return (
    <div className="flex flex-col gap-3 rounded-xl border border-[#f0e0d0] bg-[#FFF7F0] px-3.5 py-3">
      {/* Header */}
      ...
      <div className="flex flex-col gap-1.5">
        <div className="flex items-center justify-between text-[12px]">
          <span className="text-[#6B3E26]">Total de la orden</span>
          <span className="font-semibold text-[#3d1a24]">
            ${ordenTotal.toFixed(2)}
          </span>
        </div>
        <div className="flex items-center justify-between text-[12px]">
          <span className="text-[#6B3E26]">Pagado</span>
          <span className="font-semibold text-emerald-600">
            ${totalPagado.toFixed(2)}
          </span>
        </div>
        {/* Progress bar */}
        <div className="h-2 bg-white rounded-full overflow-hidden border border-[#f0e0d0]">
          <div
        ...
        <div className="flex items-center justify-between text-[12px]">
          <span className="text-[#6B3E26]">Pendiente</span>
          <span
            className={`font-bold ${
      ...
      {/* Lista de pagos */}
      {loading ? (
        <p className="text-[11px] text-[#6B3E26]">Cargando…</p>
      ) : pagos.length > 0 ? (
        <div className="flex flex-col gap-1.5">
            ...
            >
              ...
              {confirmId === p.id ? (
                <div className="flex items-center gap-1.5">
                  <span className="text-[10px] text-[#6B3E26]">¿Eliminar?</span>
                  <button
                  ...
                  <button
                    ...
                    className="px-2 py-0.5 rounded-md border border-[#e8c4a0] text-[#6B3E26] text-[10px] hover:bg-[#f0e0d0] transition"
                  >
                    No
                  </button>
                </div>
                ...
                <button
                  ...
                  className="p-1 rounded-md hover:bg-red-50 text-[#AA6A42] hover:text-red-500 transition"
        ...
        <p className="text-[11px] text-[#AA6A42] text-center py-1">
      ...
      {showForm ? (
        <div className="flex flex-col gap-2 rounded-lg border border-[#e8c4a0] bg-white px-3 py-3">
          <p className="text-[10px] font-semibold text-[#AA6A42] uppercase tracking-wider mb-0.5">
            Registrar pago
          </p>
          <div className="flex gap-2">
            <div className="flex flex-col gap-1 flex-1">
              <label className="text-[10px] text-[#6B3E26] font-medium">Fecha</label>
              <input
                type="date"
                value={fecha}
                onChange={(e) => setFecha(e.target.value)}
                className="text-[12px] border border-[#e8c4a0] rounded-lg px-2.5 py-1.5 text-[#3d1a24] focus:outline-none focus:border-[#c0607a] bg-white"
              />
            </div>
            <div className="flex flex-col gap-1 flex-1">
              <label className="text-[10px] text-[#6B3E26] font-medium">Monto ($)</label>
              <input
                type="number"
                min="0.01"
                step="0.01"
                ...
                className="text-[12px] border border-[#e8c4a0] rounded-lg px-2.5 py-1.5 text-[#3d1a24] focus:outline-none focus:border-[#c0607a] bg-white tabular-nums"
              />
            </div>
          </div>
          {errMsg && (
        ...
        !liquidado && (
          <button
            ...
            className="flex items-center justify-center gap-1.5 py-2 rounded-lg border border-dashed border-[#e8c4a0] text-[#c0607a] text-[12px] font-semibold hover:bg-white/60 transition"
          >
