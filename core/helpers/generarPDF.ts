...
async function fetchCatNames(): Promise<CatNames> {
  try {
    ...
    return {
      ...
      ornamentos: toMap(data.ornamentos ?? []),
    };
  } catch {
    return {};
  }
...
export async function buildCotizacionHtml(orden: Orden, costoEnvio?: number): Promise<string> {
  ...
  const items: OrdenPdfItem[] = orden.items.map((item) => {
    ...
    const opc = normalizeOpciones((conf.opciones ?? conf) as Record<string, any>);
    ...
    const coberturasTxt = ((opc.coberturas ?? []) as { coberturaId: string; saborCoberturaId: string | null }[])
      .map((sel) => {
        const nom = cn.coberturas?.[sel.coberturaId];
        const sab = sel.saborCoberturaId ? cn.sabores?.[sel.saborCoberturaId] : null;
        return nom ? `${nom}${sab ? ` — sabor ${sab}` : ""}` : null;
      })
      .filter(Boolean) as string[];
    if (coberturasTxt.length) detalles.push(`Cobertura: ${coberturasTxt.join(" + ")}`);

    const rellenosTxt = ((opc.rellenos ?? []) as { rellenoId: string; saborRellenoId: string | null }[])
      .map((sel) => {
        const nom = cn.coberturas?.[sel.rellenoId];
        const sab = sel.saborRellenoId ? cn.sabores?.[sel.saborRellenoId] : null;
        return nom ? `${nom}${sab ? ` — sabor ${sab}` : ""}` : null;
      })
      .filter(Boolean) as string[];
    if (rellenosTxt.length) detalles.push(`Relleno: ${rellenosTxt.join(" + ")}`);
    ...
    const orns = ((opc.ornamentos ?? []) as { ornamentoId: string; cantidad: number }[])
      .map((sel) => {
        const nom = cn.ornamentos?.[sel.ornamentoId];
        return nom ? `${nom} ×${sel.cantidad}` : null;
      })
      .filter(Boolean) as string[];
    if (orns.length) detalles.push(`Ornamentos: ${orns.join(", ")}`);
...
export function buildComandaHtml(data: ComandaData): string {
  ...
  const notasHtml = data.notas
    ? sec(
        "📝",
        "Observaciones",
        `<div style="padding:12px 16px;font-size:13px;color:${C.text};line-height:1.6;">${data.notas}</div>`,
      )
    : "";
